...
export default function TrainingSession() {
  const [activities, setActivities] = useState([
    { fase: 'Inicial', descripcion: 'Calentamiento: Juego predeportivo, donde se dividen dos equipos.' },
    { fase: 'Central', descripcion: 'Jumping jacks, Escaladoras, Sentadilla con saltos, Core (siting twists), Flexión de brazo y aplauso, Burpees, Elevación de piernas sobre cono.' },
    { fase: 'Final', descripcion: 'Posición supino (boca arriba), Posición sentado, Presión en la espalda (8”), Posición fetal (8”).' }
  ]);

  const handleEdit = (index) => {
    const newDesc = prompt('Editar descripción:', activities[index].descripcion);
    if (newDesc !== null) {
      ...
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-azul-principal">
            <th className="border border-gray-300 p-2 text-white">Fase</th>
            <th className="border border-gray-300 p-2 text-white">Descripción de Actividad</th>
            <th className="border border-gray-300 p-2 text-white">Acciones</th>
          </tr>
        </thead>
        <tbody>
          {activities.map((actividad, index) => (
            <tr key={index}>
              <td className="border border-gray-300 p-2 font-semibold">{actividad.fase}</td>
              <td className="border border-gray-300 p-2">{actividad.descripcion}</td>
              <td className="border border-gray-300 p-2 flex gap-2">
                <button onClick={() => handleEdit(index)} className="bg-blue-500 text-white px-2 py-1 rounded">Editar</button>
                <button onClick={() => handleDelete(index)} className="bg-red-500 text-white px-2 py-1 rounded">Eliminar</button>
              </td>
            </tr>
          ))}
        </tbody>
